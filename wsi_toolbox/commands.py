...
from .common import DEFAULT_MODEL, DEFAULT_BACKEND
...
class BaseCommand(ABC):
    ...
    def __init__(self, progress: str = DEFAULT_BACKEND, verbose: bool = True):
        """
        Initialize base command

        Args:
            progress: Progress bar backend ('tqdm', 'streamlit', etc.)
...
class Wsi2HDF5Command(BaseCommand):
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            **kwargs: Additional arguments for BaseCommand
        """
    ...
    def __call__(self, input_path: str, output_path: str) -> dict:
        ...
        if self.progress == 'tqdm':
            self._log(f'Original mpp: {original_mpp:.6f}')
            self._log(f'Image mpp: {mpp:.6f}')
            self._log(f'Target resolutions: {W} x {H}')
            self._log(f'Obtained resolutions: {x_patch_count*S} x {y_patch_count*S}')
            self._log(f'Scale: {scale}')
            self._log(f'Patch size: {T}')
            self._log(f'Scaled patch size: {S}')
            self._log(f'Row count: {y_patch_count}')
            self._log(f'Col count: {x_patch_count}')
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            cursor = 0
            tq = tqdm_or_st(range(row_count), backend=self.progress)
            for row in tq:
                # Read one row
        ...
        if self.progress == 'tqdm':
            self._log(f'{patch_count} patches were selected.')
